...
export const generateTokens = (userId: string, email: string, authProvider: string) => {
  const accessToken = jwt.sign({ userId, email, authProvider }, config.jwtSecret, {
    expiresIn: config.jwtExpiresIn,
  });

  const refreshToken = jwt.sign({ userId, email, authProvider }, config.jwtRefreshSecret, {
